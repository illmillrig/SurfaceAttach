...
MStatus SurfaceAttach::compute(const MPlug& plug, MDataBlock& dataBlock) {
    ...
    const int dataSamples = dataBlock.inputValue(SurfaceAttach::samples).asInt();
...
void SurfaceAttach::inUVs(MFnDependencyNode &fn) {
    ...
    const unsigned int numElements = inUVPlug.numElements();
...
void SurfaceAttach::calculateUV(const int plugID, const double &dataOffset, const double &dataReverse,
    ...
    // Offset
    const double sum = parmU + dataOffset;
    if (sum >= 0.0)
        parmU = fmod(sum, 1.0);
    else
    ...
    if (dataGenus > 0){
        ...
        const double uLength = length * (parmU * ratio);
...
double SurfaceAttach::uParmFromLength(const double distanceU) {
    const size_t index = this->binSearch(distanceU);

    const double distA = distances[index][0];
    const double uA = distances[index][1];

    const double distB = distances[index+1][0];
    const double uB = distances[index+1][1];

    const double distRatio = (distanceU - distA) / (distB - distA);
